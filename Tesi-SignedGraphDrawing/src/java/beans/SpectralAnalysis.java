...
public class SpectralAnalysis {
    ...
    private double[] getEigenVector(String type){
        ...
        for(int i=0; i<m_eigenvector.length; i++){
            m_eigenvector[i] = eigenvector.getEntry(i);
        }
        
        double max_val = max_abs_value(m_eigenvector);
        ...
        for(int i=0; i<m_eigenvector.length; i++){
            //double x_mapped = (((eigenvector.getEntry(i)-input_start)/interval)*2)-1; //mapping tra [-1,1]
            //m_eigenvector[i] = DoubleRounder.round(x_mapped, 2);
            
            //m_eigenvector[i] = DoubleRounder.round(eigenvector.getEntry(i)/max_val,2);
            m_eigenvector[i] = eigenvector.getEntry(i)/max_val;
        }
        
        return m_eigenvector;
    }

    private double max_abs_value(double[] ev){
        double max = Math.abs(ev[0]);
        for(int i=1; i<ev.length; i++){
            double v_abs = Math.abs(ev[i]);
            if(v_abs > max){
                max = v_abs;
            }
        }
    ...
    public ArrayList<Edge> getInterEdges(){
        ArrayList<Edge> inter = new ArrayList<>();
        for(Edge e:edges){
            if(e.getSource().getCluster()!=e.getTarget().getCluster()){
                inter.add(e);
            }  
